...
def _jsx_escape_path(p: str) -> str:
    """Escape a file path for an ExtendScript string literal (forward slashes, no bare quotes)."""
    return p.replace("\\", "/").replace('"', '\\"')


def _jsx_escape_value(s: str) -> str:
    """Escape a data value for an ExtendScript string literal."""
    return s.replace("\\", "\\\\").replace('"', '\\"')


def _render_jsx(placeholders: dict[str, str], template_ai_path: str, out_pdf_path: str) -> str:
    """Read runner.jsx template and inject the replacements dict + path tokens.

    The replacements dict is generated by Python and injected wholesale as a JSX
    object literal into the <<REPLACEMENTS_DICT>> slot in runner.jsx.  This avoids
    the collision bug where Python's str.replace() would corrupt both the key AND
    the value of every entry in the static dict that used to live in runner.jsx.
    """
    ...
    # Inject path tokens
    jsx_source = jsx_source.replace("<<TEMPLATE_PATH>>", _jsx_escape_path(template_ai_path))
    jsx_source = jsx_source.replace("<<OUTPUT_PDF>>", _jsx_escape_path(out_pdf_path))
